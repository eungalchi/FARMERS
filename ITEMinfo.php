  ...
  <!-- contact -->
	<div class="banner-bottom">
		...
		<!---728x90--->
			...
			<h2 class="title-w3-agileits inner">ITEM INFO</h2>
			<p class="quia">View Item Information </p>
<!---728x90--->
      <div class="more">
	...
	<?php
		...
		$kakaolink = $_REQUEST["Link"];
		$item = $_REQUEST["I_Category"];
		$Quantity = $_REQUEST["Quantity"];
		$randomcode = $_REQUEST["Code"];
		$callitem = $_REQUEST["Callitem"];
		$intro = $_REQUEST["Message"];
		$photo = basename($_FILES["photofile"]["name"]);
		move_uploaded_file($_FILES["photofile"]["tmp_name"],"/var/www/html/web/FARMERS/pictures/$photo");
		$mainphoto = "pictures/$photo";


	if ($item != null) {
		$sql = "INSERT INTO items (email, title, kakaolink, farm, crop, address1, address2, amount, randomcode, callitem, intro, picture) VALUES('$login_session', '$title','$kakaolink','$farmname','$item','$location','$address','$Quantity','$randomcode','$callitem','$intro','$mainphoto')";
		mysqli_query($conn,$sql);
		$selsql = "SELECT email,title,farm,address1,address2,crop,kakaolink,intro,picture FROM items WHERE randomcode='$randomcode'";
		$result = mysqli_query($conn,$selsql);
	}else{
		$callitem = $_REQUEST["callitem"];
		$selsql = "SELECT email,title,farm,address1,address2,crop,kakaolink,intro,picture FROM items WHERE callitem='$callitem'";
		$result = mysqli_query($conn,$selsql);
	}
	...
	?>
			...
			<div class="col-md-8 wthree_contact_left">
				...
				<h4>Info</h4>
				<form action="ITEMinfo.php?callitem=<?= $callitem?>" method="post">
            ...
            <?php print $row["intro"]; ?>
					...
					<input type="submit" name ="purchase" value="주문" onclick="myorder()">
          <input type="submit" name="shoppingbasket" value="장바구니" onclick="mybasket()">
				</form>
<!-- 관리자 MODE => 게시글 삭제 Button -->
			<?php
				if ($row1['email'] == "admin@admin.com" || $row1['email'] == $row['email']){ //관리자 or 글 게시 계정일 경우
			?>
...
<script>
	function mybasket(){
		<?php
		if(isset($_POST['shoppingbasket'])){
		$putbasket = "INSERT INTO baskets (email, callitem) VALUES('$login_session','$callitem')";
		mysqli_query($conn,$putbasket);
		}
		?>
	}
	function myorder(){
		alert("kakao openchat link : <?php print $row["kakaolink"]; ?>");
		<?php
		if(isset($_POST['purchase'])){
		$putorder = "INSERT INTO orders (email, callitem) VALUES('$login_session','$callitem')";
		mysqli_query($conn,$putorder);
		}
		?>
	}
</script>
			</br></div><!--
      <img src="양파.jpg" alt=" " width="350" height="250"/>
      <img src="양파.jpg" alt=" " width="350" height="250"/>
      <img src="양파.jpg" alt=" " width="350" height="250"/>
      <img src="양파.jpg" alt=" " width="350" height="250"/>
      <img src="양파.jpg" alt=" " width="350" height="250"/>
...
?>
            <div class="more">
              <a href="ReviewRegister.php?callitem=<?= $callitem ?>" class="hvr-shutter-in-vertical">Register<span class="glyphicon glyphicon-arrow-middle"></span></a>
            </div>
